"""Shadow directory utilities for cdx-rocks.

Workaround for rocksdict read-only bug: symlink DB files and copy config
so a read-only RocksDB mount can be opened.
"""
...
def get_rocks_dir(rocksdb_dir: str, linksdir: str) -> str:
    """Return a rocks dir where CONFIG_FILENAME is writable."""
    logger.info(f"checking {rocksdb_dir}/{CONFIG_FILENAME}  shadow {linksdir}")
    try:
...
def setup_shadow(rocksdb_dir: Path, linksdir: Path) -> Path:
    ...
    logger.info(rocksdb_dir)
    logger.info(linksdir)
